...
export const MakeUpExamsView: React.FC<MakeUpExamsViewProps> = ({
  ...
  // Sorting state for every table column
  type SortField =
    | 'no'
    | 'studentName'
    | 'className'
  ...
  // Otomatis sinkronkan siswa susulan jika Daftar Hadir atau Berita Acara berubah
  React.useEffect(() => {
    StorageService.syncMakeUpExams();
  }, [attendances, propMinutes]);
  ...
  // Sorted records based on chosen column and direction
    ...
    list.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'subjectName':
          ...
          break;
        case 'originalDate':
          valA = `${a.originalDate || ''}_${a.originalSession || ''}`;
          valB = `${b.originalDate || ''}_${b.originalSession || ''}`;
          break;
        case 'reason':
          valA = a.reason || '';
          valB = b.reason || '';
        ...
        case 'status':
          ...
          break;
        case 'makeUpDate':
          valA = `${a.makeUpDate || ''}_${a.roomName || ''}`;
          valB = `${b.makeUpDate || ''}_${b.roomName || ''}`;
          break;
        case 'no':
        default:
          return 0;
  ...
  const handleExportExcel = () => {
    ...
    const dataToExport = sortedRecords.map((rec, index) => ({
      No: index + 1,
        ...
        <div className="print-page-a4 bg-white p-6 md:p-8 max-w-5xl mx-auto text-black font-serif text-xs shadow-md print:shadow-none print:p-0">
          ...
          <div className="mt-4 space-y-3 leading-relaxed">
            ...
            <div className="pt-8 grid grid-cols-2 gap-8 text-center text-xs print-avoid-break">
              ...
              <div>
                <p>{settings.city || 'Kota'}, {new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
        ...
        <>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button
                ...
                onClick={() => {
                  StorageService.syncMakeUpExams();
                  onRefresh();
                  setSuccessMessage('Daftar siswa susulan berhasil diperbarui otomatis dari Berita Acara dan Daftar Hadir.');
                  setTimeout(() => setSuccessMessage(null), 3000);
                }}
                className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-300 rounded-md text-xs font-semibold flex items-center gap-1.5 shadow-xs transition-colors cursor-pointer"
                title="Perbarui otomatis dari Daftar Hadir & Berita Acara"
              >
                <RotateCcw className="w-3.5 h-3.5 text-slate-600" />
                <span>Sinkronkan Otomatis</span>
              </button>
              ...
              <button
                ...
                className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-xs font-semibold flex items-center gap-1.5 shadow-xs transition-colors cursor-pointer"
              ...
              {makeUpExams.length > 0 && (
                <button
                  ...
                  className="px-2.5 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded-md text-xs font-medium flex items-center gap-1.5 transition-colors cursor-pointer"
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex flex-wrap items-center justify-between gap-3 shadow-xs">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-amber-900">
                  siswa susulan dipilih dari {sortedRecords.length} yang ditampilkan
                </span>
              </div>
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3 py-1.5 bg-white hover:bg-slate-100 text-slate-700 rounded-lg text-xs font-medium border border-slate-200 transition-colors cursor-pointer"
                >
                  Batalkan Pilihan
                </button>
                <button
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
                ...
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 uppercase font-semibold text-[11px]">
                    ...
                    <th
                      ...
                      className="p-3 w-12 text-center cursor-pointer select-none transition-colors hover:bg-slate-100/80"
                      title="Klik untuk mengurutkan berdasarkan Nomor"
                    >
                    ...
                    <th
                      ...
                      title="Klik untuk mengurutkan berdasarkan Nama Peserta"
                    >
                    ...
                    <th
                      ...
                      className="p-3 cursor-pointer select-none transition-colors hover:bg-slate-100/80"
                      title="Klik untuk mengurutkan berdasarkan Jadwal Utama"
                    >
                      <div className="flex items-center gap-1">
                        <span>Jadwal Utama</span>
                        {renderSortIcon('originalDate')}
                      </div>
                    </th>
                    <th
                      onClick={() => handleSort('reason')}
                      className="p-3 cursor-pointer select-none transition-colors hover:bg-slate-100/80"
                      title="Klik untuk mengurutkan berdasarkan Alasan Ketidakhadiran"
                    >
                      <div className="flex items-center gap-1">
                        <span>Alasan Ketidakhadiran</span>
                    ...
                    <th
                      ...
                      className="p-3 cursor-pointer select-none transition-colors hover:bg-slate-100/80"
                      title="Klik untuk mengurutkan berdasarkan Status Susulan"
                    >
                      <div className="flex items-center gap-1">
                        <span>Status Susulan</span>
                        {renderSortIcon('status')}
                      </div>
                    </th>
                    <th
                      onClick={() => handleSort('makeUpDate')}
                      className="p-3 cursor-pointer select-none transition-colors hover:bg-slate-100/80"
                      title="Klik untuk mengurutkan berdasarkan Jadwal Pelaksanaan Susulan"
                    >
                      <div className="flex items-center gap-1">
                        <span>Jadwal Pelaksanaan</span>
                        {renderSortIcon('makeUpDate')}
                      </div>
                    </th>
                          ...
                          <td className="p-3 text-right">
                            <div className="flex items-center justify-end gap-1.5">
                              <button
                                ...
                                className="px-2.5 py-1 bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200 rounded-md font-medium text-[11px] flex items-center gap-1 transition-colors cursor-pointer"
            ...
            <div className="p-3 bg-slate-50 border-t border-slate-200 flex flex-wrap items-center justify-between text-xs text-slate-500 gap-2">
              <span>
                Menampilkan <strong>{sortedRecords.length}</strong> dari total{' '}
